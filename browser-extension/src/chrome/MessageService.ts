...
export class MessageService implements IMessageService {
  ...
  protected async handleChromeQuery(
    query: any,
    ...
    if (valid) {
      ...
      await Promise.all(
        handlers.map(async (handler) => {
          if (
            handler.messageTypes.findIndex(
              (type) => validated.type === type,
            ) !== -1
          ) {
            try {
              const { status, message } = await handler.handleMessage(
                validated.message,
                sender,
              )
              if (status === Status.ok) {
  ...
  protected constructErrorResponse(
    query: ChromeQuery,
    message?: string,
  ): ChromeResponse {
